...
def get_words(raw_image):
	...
	for i in range ( 0, len(ycoords)-1 ): #iterate line
		...
		_,contour0,_ = cv2.findContours(line.copy(),cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_SIMPLE)
	...
	for i in range ( 0, len(ycoords)-1 ): #iterate line
		...
		cv2.imwrite('img_line'+str(i)+'.png', line)
		...
		#cv2.imwrite('img_words'+str(i)+'.png', line)
		...
		for j in range (0, len(xcoords)-1 ): #iterate words
			...
			cv2.imwrite('img_words'+str(number_of_words)+'.png', word)
