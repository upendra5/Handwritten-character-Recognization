...
def get_characters(raw_image,max_line_height,line,word):
	...
	_,contour0,_ = cv2.findContours(mo_image.copy(),cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_SIMPLE)
	...
	#for x,y,w,h in rect :
        #        img_ch=mo_image[y:y+h,x:x+w]
        #        cv2.imwrite(str(num)+'char.png',img_ch)
	...
	for i in rect:
		...
		cv2.imwrite('img_char'+str(i)+'.png',letter)
		num=num+1
		print letter
